...
public class CertManagerDataValidator implements DataValidator {
			...
			if (!modifyinstallation) {
				...
				if (certCreate) {
					...
					adata.setVariable("syracuse.certificate.certtool", adata.getVariable("INSTALL_PATH")
							+ File.separator + "syracuse" + File.separator + "certs_tools");
				...
				if (!modifyinstallation) {
					...
					if (certCreate) {
						if (mongodbInstall) {
							...
							adata.setVariable("mongodb.service.hostname",
	...
	public void clientPutInPlace(InstallData adata, String strFieldCa) throws Exception {
	...
	public void readCerts(InstallData adata) throws Exception {
		...
		File certToolOutputServerCRT = new File(
				strCertToolPath + File.separator + "output" + File.separator + localHOST_NAME + ".crt");
		...
		File sourceserverKey = new File(fieldPemKeyFile);
		File certToolOutputServerkey = new File(
				strCertToolPath + File.separator + "output" + File.separator + localHOST_NAME + ".key");
		...
		File certToolOutputServerPub = new File(
				strCertToolPath + File.separator + "output" + File.separator + localHOST_NAME + ".pem");
...
//        }
	...
	public void certCreate(InstallData adata) throws Exception {
		...
		KeyPairGeneratorDataValidator.writePrivateKey(
				strCertToolPath + File.separator + "private" + File.separator + "ca.cakey", pairCA,
				capassphrase.toCharArray());
		...
		X509Certificate servercert = CreateCertsValidator.generateServerV3Certificate(pairServer, countryCode,
				organization, organizationalUnit, state, city, hostname, hostname, null, validity, cacert, pairCA);
		...
		KeyPairGeneratorDataValidator.writePrivateKey(strCertPath + File.separator + localHOST_NAME + ".key",
				pairServer, serverpassphrase.toCharArray());
		File certToolOutputServerkey = new File(
				strCertToolPath + File.separator + "output" + File.separator + localHOST_NAME + ".key");
		File certsServerkey = new File(strCertPath + File.separator + localHOST_NAME + ".key");

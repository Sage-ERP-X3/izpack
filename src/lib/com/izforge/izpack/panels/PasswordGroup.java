...
//  private boolean hasParams = false;
	...
	public PasswordGroup(InstallData idata, List<ValidatorContainer> validatorContainers, String processor) {
		...
		if (processor != null) {
			try {
				...
				Debug.trace("Failed in PasswordGroup constructor making processor Class.forName('" + processor + "'): "
						+ exception);
	 ...
	 * of a validation rule exists. Otherwise <code>false</code> is returned.
	 */
	/*--------------------------------------------------------------------------*/
	...
	/*--------------------------------------------------------------------------*/
	/**
	 * Returns the password in the following order: If a validator sets a
	 * modified password such as an encrypted string that is returned, OR if a
	 * processing service class was supplied it will be used to process the
	 * password before it is returned, otherwise the content of the first field
	 * will be returned.
	 *
	 * @return the password
	 */

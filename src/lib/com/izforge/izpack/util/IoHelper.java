...
public class IoHelper {
	...
	/**
	 * Creates an in- and output stream for the given File objects and copies all
	 * the data from the specified input to the specified output.
	 *
	 * @param inFile
	...
	/**
	 * Creates an in- and output stream for the given File objects and copies all
	 * the data from the specified input to the specified output. If permissions is
	 * not null, a chmod will be done on the output file.
	 *
	 * @param inFile
	 *            File object for input
	 * @param outFile
	...
	/**
	 * Creates an in- and output stream for the given File objects and copies all
	 * the data from the specified input to the specified output. If the
	 ...
	 * Creates an in- and output stream for the given File objects and copies all
	 * the data from the specified input to the specified output. If the
	 * VariableSubstitutor is not null, a substition will be done during copy. If
	 ...
	 *             if an I/O error occurs
	 */
	public static void copyFile(File inFile, File outFile, String permissions, VariableSubstitutor vs)
			throws IOException {
	 ...
	 * the data from the specified input to the specified output. If the
	 * VariableSubstitutor is not null, a substition will be done during copy. If
	 * permissions is not null, a chmod will be done on the output file. If type is
	 * not null, that type is used as file type at substitution.
	 *
	 * @param inFile
	 *            File object for input
	 * @param outFile
	 ...
	 *             if an I/O error occurs
	 */
	public static void copyFile(File inFile, File outFile, String permissions, VariableSubstitutor vs, String type)
			throws IOException {
	...
	/**
	 * Copies an input stream to an output stream. Both streams must already be
	 * open. If the VariableSubstitutor is not null, a substition will be done
	 * during copy.
	 *
	 * @param in
	 *            stream object for input
	 * @param out
	 ...
	 *             if an I/O error occurs
	 */
	public static void copyStream(InputStream in, OutputStream out, VariableSubstitutor vs, String type)
			throws IOException {
		if (vs == null) {
	...
	/**
	 * Creates a temp file with delete on exit rule. The extension is extracted from
	 * the template if possible, else the default extension is used. The contents of
	 * template will be copied into the temporary file.
	 *
	 * @param template
	...
	/**
	 * Creates a temp file with delete on exit rule. The contents of the input
	 * stream will be copied into the temporary file. If the variable substitutor is
	 * not null, variables will be replaced during copying.
	 *
	 * @param is
	 ...
	 * Creates a temp file with delete on exit rule. The extension is extracted from
	 * the template if possible, else the default extension is used. The contents of
	 * template will be copied into the temporary file. If the variable substitutor
	 ...
	 * @throws IOException
	 */
	public static File copyToTempFile(File template, String defaultExtension, VariableSubstitutor vss)
			throws IOException {
	 ...
	 *
	 * @param path
	 *            the absolute path of the file for which the permissions should be
	 *            changed
	 * @param permissions
	 *            POSIX permissions to be set
	 * @throws IOException
	...
	/**
	 * Returns the free (disk) space for the given path. If it is not ascertainable
	 * -1 returns.
	 *
	 * @param path
	 ...
	 * environment. Some methods of this class are not supported on all operation
	 * systems.
	 *
	 * @param method
	 *            name of the method
	...
	/**
	 * Extracts a long value from a string in a special manner. The string will be
	 * broken into tokens with a standard StringTokenizer. Arround the assumed place
	 * (with the given half range) the tokens are scaned reverse for a token which
	 * represents a long. if useNotIdentifier is not null, tokens which are contains
	 * this string will be ignored. The first founded long returns.
	 *
	 * @param in
	...
	/**
	 * Returns the primary group of the current user. This feature will be supported
	 * only on Unix. On other systems null returns.
	 *
	 ...
	 * string with with. In opposite to the String.replaceAll method this method do
	 * not use regular expression or other methods which are only available in JRE
	 * 1.4 and later. This method was special made to mask masked slashes to avert a
	 * conversion during path translation.
	 *
	 * @param destination
	 ...
	 * @return a new String object if what was found in the given string, else the
	 *         given string self
	 */
	...
	public static String translatePath(String destination, VariableSubstitutor vs) {
		...
		if (destination.indexOf("\\/") >= 0 && destination.indexOf(MASKED_SLASH_PLACEHOLDER) < 0) { // Masked slash
																									// found,
																									// placeholder not
																									// included in
																									// destination ->
	...
	/**
	 * Returns the value of the environment variable given by key. This method is a
	 * work around for VM versions which do not support getenv in an other way. At
	 * the first call all environment variables will be loaded via an exec. On
	 * Windows keys are not case sensitive.
	 *
	 * @param key
	...
	public static String getRegistry(String keyRegistryString) {
		if (OsVersion.IS_WINDOWS) {
			try {
				String[] registryItems = keyRegistryString.split(":");
				String regHKey = registryItems[0];
				String regKeyPath = registryItems[1];
				String regValueName = registryItems[2];
	...
	/**
	 * Extracts key and value from the given string var. The key should be separated
	 * from the value by a sign. On Windows all chars of the key are translated to
	 * upper case.
	 *
	 * @param var
	 */
